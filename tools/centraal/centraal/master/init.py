...
def main():
    ...
    if args.private_key_file is not None:
        ...
        if not os.path.isfile(private_key_file):
            print(
                "ssh private key does not exist: " + private_key_file, file=sys.stderr
            )
            sys.exit(1)
    else:
        ...
        if host not in ssh_config_manager.get_keys():
            ...
            ssh_config_manager.add_key_if_not_exists(host, password=ssh_key_password)
            subprocess.run(
                [
                    "ssh-copy-id",
                    "-i",
    ...
    try:
        ...
        client.connect(
            host,
            username=user,
            pkey=Ed25519Key.from_private_key_file(private_key_file, ssh_key_password),
            port=port,
        )
        ...
        print(
            "authentication failed. maybe you have already inited your master cluster (root user ssh is disabled after that)?",
            file=sys.stderr,
        )
    finally:
        client.close()
